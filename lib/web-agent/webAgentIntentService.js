import { extractQuantityFromText } from "./telvoiceQuoteService.js";
import {
  hasCommercialIntent,
  isGreetingOnly,
  normalizeCommercialText,
  normalizeIntentText as normalizeCommercialIntentText,
} from "./commercialText.js";
import { isLandingFaqQuestion } from "./faq.js";
import { matchesCapabilitiesIntent } from "./capabilities.js";
import {
  isHighVolumeQualifierOnly,
  matchesHighVolumeIntent,
} from "./highVolume.js";

export function normalizeIntentText(text) {
  return String(text || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9\s]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

const COMMERCIAL_HINT =
  /\b(comprar|bolsa|bolsas|cotizar|precios|planes|sms|mensajes|mensaje|mercadopago|chile|empresa|ecommerce|retail|otp|campaña|campana|mas sms|necesito sms|recargar|paquete)\b/;

export function classifyWebAgentIntent(text) {
  const commercialNormalized = normalizeCommercialText(text);
  const normalized = normalizeCommercialIntentText(commercialNormalized);
  const quantity = extractQuantityFromText(text);

  if (!normalized) {
    return { intent: "fallback", quantity };
  }

  if (matchesCapabilitiesIntent(normalized)) {
    return { intent: "capabilities", quantity };
  }

  if (
    matchesHighVolumeIntent(normalized) &&
    isHighVolumeQualifierOnly(normalized, quantity)
  ) {
    return { intent: "high_volume", quantity };
  }

  if (isGreetingOnly(text)) {
    return { intent: "greeting", quantity };
  }

  if (
    normalized === "portal" ||
    /\b(ir al portal|acceder al portal|ingresar al portal|entrar al portal|panel cliente|mi cuenta)\b/.test(
      normalized,
    ) ||
    /\bportal\b/.test(normalized)
  ) {
    return { intent: "portal", quantity };
  }

  if (hasCommercialIntent(text)) {
    if (quantity !== null || /\b(cotizar|cotiza|cuanto cuesta)\b/.test(normalized)) {
      return { intent: "quote", quantity };
    }
    if (/\b(precios|planes|tarifas|ver precios)\b/.test(normalized)) {
      return { intent: "prices", quantity };
    }
    if (/\b(pagar|pago|mercadopago|link de pago|checkout)\b/.test(normalized)) {
      return { intent: "payment", quantity };
    }
    if (/\b(registr|crear cuenta|abrir cuenta|portal cliente)\b/.test(normalized)) {
      return { intent: "register", quantity };
    }
    if (/\b(asesor|ejecutivo|humano|contactar|ventas)\b/.test(normalized)) {
      return { intent: "advisor", quantity };
    }
    return { intent: "purchase", quantity };
  }

  if (/\b(registr|crear cuenta|abrir cuenta|portal cliente)\b/.test(normalized)) {
    return { intent: "register", quantity };
  }

  if (/\b(pagar|pago|mercadopago|link de pago|checkout)\b/.test(normalized)) {
    return { intent: "payment", quantity };
  }

  if (/\b(asesor|ejecutivo|humano|contactar|ventas)\b/.test(normalized)) {
    return { intent: "advisor", quantity };
  }

  if (isLandingFaqQuestion(text)) {
    return { intent: "faq", quantity };
  }

  if (quantity !== null || /\b(cotizar|cotiza|cuanto cuesta)\b/.test(normalized)) {
    return { intent: "quote", quantity };
  }

  if (
    /\b(quiero comprar|comprar sms|comprar bolsa|comprar mas|necesito sms|necesito enviar|mas sms|necesito mensajes|quiero mensajes|necesito una bolsa|recargar)\b/.test(
      normalized,
    )
  ) {
    return { intent: "purchase", quantity };
  }

  if (/\b(precios|planes|tarifas|ver precios)\b/.test(normalized)) {
    return { intent: "prices", quantity };
  }

  if (
    /\b(como funciona|que es telvoice|ayuda|faq|otp|api|operadores|factura|despues de comprar|activar|bolsa sms)\b/.test(
      normalized,
    ) ||
    /\b(que es|como se|sirve para)\b/.test(normalized)
  ) {
    return { intent: "faq", quantity };
  }

  if (COMMERCIAL_HINT.test(normalized)) {
    return { intent: "purchase", quantity };
  }

  return { intent: "fallback", quantity };
}

export function isCommercialPriorityIntent(intent) {
  return [
    "purchase",
    "quote",
    "prices",
    "payment",
    "register",
    "portal",
  ].includes(intent);
}

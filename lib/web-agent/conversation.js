...
import { hasGreetingPrefix } from "./commercialText.js";
import { classifyWebAgentIntent } from "./webAgentIntentService.js";
import {
  calculateQuote,
  calculateQuoteAsync,
  ensurePricingTiersLoaded,
...
export const QUICK_ACTIONS = [
  ...
  { id: "portal", label: "Ir al portal" },
];

const REGISTER_URL = "https://portal.telvoice.net/";

export function buildCtas(quote) {
  const ctas = [];
  if (quote) {
    ctas.push({
      type: "pay",
...
function mapQuickAction(actionId) {
  const map = {
    ...
    validate_account:
      "Con numeración Telvoice puedes validar cuentas por SMS de forma privada: registro a un número exclusivo, OTP autorizado y trazabilidad empresarial — sin usar el celular personal de un gerente.\n\n¿Es para clientes finales, equipos internos o una integración?",
    own_number:
      "Telvoice asigna números SMS exclusivos para agentes, bots y operación empresarial: envío, recepción, campañas y validaciones autorizadas.\n\n¿Para qué caso lo necesitas (OTP, campañas, soporte, agentes IA)?",
    receive_sms:
      "Con numeración Telvoice puedes recibir respuestas SMS en un canal propio de tu empresa: centraliza mensajes de clientes, equipos o procesos sin mezclarlos con un celular personal.\n\n¿Qué tipo de respuestas necesitas capturar?",
...
export async function handleWebAgentTurn({
  ...
  await ensurePricingTiersLoaded();
  ...
  if (
    clientLeadCaptureStep &&
    isSupportCaptureStep(clientLeadCaptureStep)
  ) {
    if (!lead_capture_step || !isSupportCaptureStep(lead_capture_step)) {
      lead_capture_step = clientLeadCaptureStep;
    }
  }
  if (clientLeadData && typeof clientLeadData === "object") {
    Object.assign(leadData, clientLeadData);
  }
  if (
    clientLeadCaptureStep &&
    isSupportCaptureStep(clientLeadCaptureStep) &&
  ...
  if (quickAction) {
    ...
    if (mapped) {
      ...
      if (quickAction === "portal" || quickAction === "purchase") {
        return packResponse(
          session.id,
          reply,
          quickAction === "purchase" ? "purchase" : "portal",
          session.last_quote,
          null,
          false,
          [{ type: "register", label: "Ir al portal", url: REGISTER_URL }],
  ...
  if (lead_capture_step && isSupportCaptureStep(lead_capture_step)) {
    const parsed = parseSupportStepInput(lead_capture_step, userText, leadData);
    if (!parsed.ok) {
      reply = parsed.error + "\n\n" + getSupportStepPrompt(lead_capture_step);
      await appendMessage(session.id, "assistant", reply, {
        lead_step: lead_capture_step,
        intent: "support",
      });
      return packResponse(
        session.id,
        ...
        true,
        null,
        packSupportLeadData(leadData),
      );
    }

    const updatedLead = parsed.leadData;
    const step = nextSupportStep(lead_capture_step);
    ...
    if (step) {
      ...
      return packResponse(
        ...
        true,
        null,
        packSupportLeadData(updatedLead),
      );
    }
    ...
    try {
      const result = await submitSupportRequest({
        ...
        issue: updatedLead.issue,
        pageUrl: pageUrl || landingPage,
        sessionId: session.id,
      });
      reply =
        result?.message ||
        "Listo. Enviamos tu consulta a soporte Telvoice. Te contactaremos pronto al correo que indicaste.";
    } catch (err) {
      console.error("[web-agent] support submit:", err.message);
      reply =
        "No pudimos enviar tu consulta en este momento. Intenta de nuevo en unos minutos o usa el formulario de contacto del sitio.";
    }
  ...
  if (
    userText &&
    isSupportRequest(userText) &&
    !isLandingFaqQuestion(userText) &&
    !isSupportCaptureStep(lead_capture_step)
  ) {
    lead_capture_step = "support_name";
    const nextLead = { ...leadData, support_flow: true };
    delete nextLead.pending_action;
    await updateSession(session.id, {
      lead_capture_step: "support_name",
      lead_data: nextLead,
    });
    reply =
      "Te ayudo a enviar tu consulta a soporte Telvoice.\n\n" +
      getSupportStepPrompt("support_name");
    intent = "support";
    lead_required = true;
    await appendMessage(session.id, "assistant", reply, { intent: "support" });
    return packResponse(
      session.id,
      reply,
      intent,
  ...
  const changeQty = /cambiar\s+cantidad/i.test(userText);
  if (changeQty) {
    reply =
      "Indica la nueva cantidad de SMS que quieres cotizar (ej. 15000 o 30000).";
    intent = "quote";
    const nextLead = {
      ...leadData,
      pending_action: "quote_quantity",
    ...
    return packResponse(session.id, reply, intent, session.last_quote, null, false);
  }
  ...
  if (intent === "greeting") {
    ...
    reply = answerFaq(userText);
  } else if (intent === "portal" || intent === "register") {
    reply = mapQuickAction("portal");
    intent = "portal";
    await appendMessage(session.id, "assistant", reply, { intent: "portal" });
    return packResponse(
      session.id,
      reply,
      intent,
    ...
    if (!qty) {
      reply = hasGreetingPrefix(userText)
        ? "Hola, perfecto. Te ayudo a comprar una bolsa de SMS para Chile.\n\n¿Cuántos SMS quieres comprar?\nPor ejemplo: 5.000, 15.000, 30.000, 50.000 o 100.000 SMS."
        : "Perfecto, te ayudo a comprar una bolsa SMS para Chile. ¿Cuántos SMS necesitas?";
      intent = "purchase";
      const nextLead = {
        ...leadData,
        pending_action: "purchase_quantity",
    ...
    if (!quote) {
      reply =
        "Primero indica cuántos SMS necesitas (ej. 30000). Luego te muestro la cotización con botón Ir a pagar.";
    } else {
      reply =
...
function packResponse(
  ...
  const leadData =
    leadDataForClient ||
    (isSupportCaptureStep(lead_capture_step) ? { support_flow: true } : null);
  ...
  return {
    ...
    lead_data: leadData,
    ctas: ctasOverride || buildCtas(quote),
  };
}

function packSupportLeadData(leadData) {
  if (!leadData?.support_flow) {
    return null;
  }
  return {
    support_flow: true,
...
export async function handleWebAgentQuoteOnly(body) {
  ...
  const quote = await calculateQuoteAsync(qty);

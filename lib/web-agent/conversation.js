...
export const WELCOME =
  "Hola, soy el agente comercial de Telvoice.cl. Te ayudo a cotizar bolsas SMS para Chile, resolver dudas del servicio, registrarte y avanzar al pago. ¿Qué necesitas hoy?";
...
function buildCtas(quote, leadComplete) {
  const ctas = [];
  if (quote) {
    ctas.push({
      type: "pay",
  ...
  if (!leadComplete) {
    ctas.push({ type: "lead", label: "Dejar mis datos" });
  }
...
export async function handleWebAgentTurn({
  ...
  if (intent === "greeting") {
    ...
    const qty = qtyDetected ?? extractQuantityFromText(userText);
    if (!qty) {
      reply =
        "Perfecto, puedo ayudarte a comprar una bolsa SMS para Chile. Las bolsas se calculan en múltiplos de 1.000 SMS y el precio baja según volumen. ¿Cuántos SMS necesitas comprar?";
      intent = "purchase";

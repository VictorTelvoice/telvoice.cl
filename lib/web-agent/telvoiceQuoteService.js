...
export function calculateQuote(requestedQuantity) {
  const norm = normalizeSmsQuantity(requestedQuantity);
...
export function formatQuoteForChat(quote) {
  ...
  if (
    ...
    lines.push(
      "Las bolsas Telvoice.cl se calculan en múltiplos de 1.000 SMS.",
      `Cantidad solicitada: ${fmt(quote.requested_quantity)} SMS`,
      `Para ${fmt(quote.requested_quantity)} SMS, la cotización se ajusta a ${fmt(quote.quoted_quantity)} SMS.`,
      "",
    );
  } else {
...
export function formatPricesCatalogMessage() {
  return (
    ...
    "• 70.000 SMS → $6/SMS → $499.800 IVA incluido\n\n" +
    "Indica cuántos SMS necesitas y te preparo la cotización."
  );
}

...
/** Términos de compra/cotización (texto ya normalizado con normalizeCommercialText). */
const COMMERCIAL_INTENT_RE =
  /\b(comprar|necesito|quiero|cotizar|cotiza|precio|precios|cuesta|cuanto cuesta|bolsa|bolsas|paquete|paquetes|sms|campanas|campana|saldo|recargar|mas sms|enviar campanas|enviar campana)\b/;

/**
 * True si el mensaje expresa intención comercial aunque incluya saludo.
 * Usar normalizeCommercialText antes de evaluar patrones de negocio.
 */
export function hasCommercialIntent(text) {
  ...
  return COMMERCIAL_INTENT_RE.test(normalized);
}

/** Saludo sin intención accionable (solo bienvenida). */
export function isGreetingOnly(text) {
  const normalized = normalizeCommercialText(text);
  if (!normalized || hasCommercialIntent(text)) {
    return false;
  }
  return /^(hola|buenos dias|buenas tardes|buenas noches|buenas|hey)(\s+que tal)?$/.test(
    normalized,
  );
}

export function hasGreetingPrefix(text) {
  const normalized = normalizeIntentText(text);
  return /^(hola|buenos dias|buenas tardes|buenas noches|buenas|hey)\b/.test(normalized);
}

(function () {
  ...
  function renderHeader(active) {
    if (!window.TELVOICE_SITE_CHROME) return "";
    return window.TELVOICE_SITE_CHROME.renderNav({
      root: root(),
      active: active === "home" ? "ayuda" : active,
    });
  }

  function renderFooter() {
    if (!window.TELVOICE_SITE_CHROME) return "";
    return window.TELVOICE_SITE_CHROME.renderFooter({ root: root() });
  }
  ...
  function mountShell(active) {
    ...
    if (window.TELVOICE_SITE_CHROME) {
      window.TELVOICE_SITE_CHROME.initNav();
      window.TELVOICE_SITE_CHROME.loadAgent(root());
    }
